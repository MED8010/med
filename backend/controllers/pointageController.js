...
const createPointage = async (req, res) => {
  try {
    ...
    const finalDate = scanner_action ? serverDate : (date || serverDate);
    ...
    if (!pointage) {
      pointage = new Pointage({
        employe: employe_id,
        date: new Date(finalDate),
        heure_entree: (effectiveAction === 'entree') ? serverTime : (heure_entree || null),
        heure_sortie: (effectiveAction === 'sortie') ? serverTime : (heure_sortie || null),
    ...
    const responsePointage = pointage.toObject();
    responsePointage.effectiveAction = effectiveAction;

    res.status(201).json({ message: 'Pointage enregistré avec succès', pointage: responsePointage });
  } catch (error) {
