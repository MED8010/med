...
const createPointage = async (req, res) => {
  try {
    ...
    // Determine effective action if 'auto' is specified
    let effectiveAction = scanner_action;
    if (scanner_action === 'auto') {
      const pointageToday = await Pointage.findOne({
        employe: employe_id,
        date: new Date(serverDate)
      });
      effectiveAction = (!pointageToday || !pointageToday.heure_entree) ? 'entree' : 'sortie';
    }
    ...
    // Attach effective action for frontend feedback
