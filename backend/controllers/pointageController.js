...
const createPointage = async (req, res) => {
  try {
    ...
    // Use manual date construction to avoid timezone shifts from toISOString
    ...
    if (scanner_action === 'auto') {
      const checkPointage = await Pointage.findOne({
        employe: employe_id,
        date: new Date(serverDate)
      });
      // If no pointage today or no entry time, assign 'entree'
      if (!checkPointage || !checkPointage.heure_entree) {
