...
const createPointage = async (req, res) => {
  try {
    ...
    // Logic for auto-detecting action
    let effectiveAction = scanner_action;

    let pointage = await Pointage.findOne({
      employe: employe_id,
      date: new Date(scanner_action ? serverDate : date)
    });
    ...
    if (scanner_action === 'auto') {
      if (!pointage || !pointage.heure_entree) {
        ...
        // If both exist, we might want to update the exit time or just keep it as is
        // For simplicity in auto mode, if both exist, we stay in 'sortie' mode to update the last exit
        effectiveAction = 'sortie';
      }
    }
    ...
    // Add effective action to the response for frontend feedback
