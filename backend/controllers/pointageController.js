...
const createPointage = async (req, res) => {
  try {
    ...
    const finalDate = (scanner_action && scanner_action !== 'auto') || scanner_action === 'auto' ? serverDate : date;
    ...
    res.status(201).json({
      ...
      pointage,
      effectiveAction: (scanner_action === 'auto') ? effectiveAction : scanner_action
    });
  } catch (error) {
