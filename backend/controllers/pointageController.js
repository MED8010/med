...
const createPointage = async (req, res) => {
  try {
    ...
    if (scanner_action === 'auto') {
      // Logic for auto: if no entry today OR already has an entry but no exit, determine based on presence
    ...
    res.status(201).json({
      ...
      pointage,
      effectiveAction // Return the action taken if it was 'auto'
    });
  } catch (error) {
