...
const createPointage = async (req, res) => {
  try {
    ...
    if (scanner_action === 'auto') {
      // Logic: assign 'entree' if no record or no entry time exists for the day, otherwise 'sortie'
    ...
    res.status(201).json({ message: 'Pointage enregistré avec succès', pointage, effectiveAction });
  } catch (error) {
