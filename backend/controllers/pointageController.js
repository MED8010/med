...
const createPointage = async (req, res) => {
  try {
    ...
    // Use local date to avoid timezone shifts with toISOString()
    ...
    // Handle automated pointage mode
    ...
    // If scanner_action is provided (or derived from 'auto'), use server time and ignore client time
    const isScanner = !!effectiveAction;
    const finalDate = isScanner ? serverDate : date;
