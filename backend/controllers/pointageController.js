...
const createPointage = async (req, res) => {
  try {
    ...
    const isAuto = scanner_action === 'auto';
    const finalDate = (scanner_action || isAuto) ? serverDate : date;
    ...
    if (isAuto) {
      // Logic: If no pointage or no entry time, it's an entry. Otherwise, it's an exit.
    ...
    res.status(201).json({
      ...
      pointage,
      effectiveAction // Return the action taken (useful for UI in auto mode)
    });
  } catch (error) {
