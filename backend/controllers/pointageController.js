...
const createPointage = async (req, res) => {
  try {
    ...
    const finalDate = (scanner_action && scanner_action !== 'manual') ? serverDate : date;

    // Normalize date to avoid timezone issues: YYYY-MM-DD
    const normalizedDate = new Date(finalDate);
