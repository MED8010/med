...
const createPointage = async (req, res) => {
  try {
    ...
    // Logic for 'auto' mode: determine if it's entry or exit
    let effectiveAction = scanner_action;
    if (scanner_action === 'auto') {
      effectiveAction = (pointage && pointage.heure_entree) ? 'sortie' : 'entree';
    }
    ...
    // Return pointage with effectiveAction for frontend feedback
    const pointageObj = pointage.toObject();
    pointageObj.effectiveAction = effectiveAction;

    res.status(201).json({
      message: 'Pointage enregistré avec succès',
      pointage: pointageObj
    });
  } catch (error) {
