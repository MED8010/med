...
const createPointage = async (req, res) => {
  try {
    ...
    // If scanner_action is 'auto', determine if it's entry or exit
    ...
    if (scanner_action === 'auto') {
      ...
      // If no pointage today or no entry yet, it's an entry. Otherwise, it's a departure.
    ...
    const finalDate = (scanner_action && scanner_action !== 'auto') || effectiveAction ? serverDate : date;
