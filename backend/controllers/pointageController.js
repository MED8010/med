...
const createPointage = async (req, res) => {
  try {
    ...
    if (scanner_action === 'auto') {
      // Logic: if no pointage today or no entry time -> it's an entry. Otherwise it's an exit.
      effectiveAction = (!pointage || !pointage.heure_entree) ? 'entree' : 'sortie';
    }

    const finalHeureEntree = (effectiveAction === 'entree') ? serverTime : (effectiveAction === 'sortie' ? (pointage?.heure_entree || '') : heure_entree);
    const finalHeureSortie = (effectiveAction === 'sortie') ? serverTime : (effectiveAction === 'entree' ? '' : heure_sortie);
