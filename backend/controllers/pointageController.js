...
const createPointage = async (req, res) => {
  try {
    ...
    const finalHeureEntree = (effectiveAction === 'entree') ? serverTime : (scanner_action ? undefined : heure_entree);
    const finalHeureSortie = (effectiveAction === 'sortie') ? serverTime : (scanner_action ? undefined : heure_sortie);
